...
reject:
    ...
    //poly_ntt(&chat);
    sppoly sp_c;
    extract_sparse_binpoly(&sp_c, &chat);
    for (i = 1; i < L; ++i) {
        //poly_pointwise_montgomery(&cs1.vec[i], &chat, &s1.vec[i - 1]);
        //poly_invntt_tomont(&cs1.vec[i]);
        index_based_sparse_binary_challenge_polymult_modQ(&cs1.vec[i], &s1.vec[i-1], &sp_c);
    }
    //polyveck_poly_pointwise_montgomery(&cs2, &s2, &chat);
    //polyveck_invntt_tomont(&cs2);
    for(i = 0; i < K; i++)
    {
        //poly_pointwise_montgomery(&cs2.vec[i], &s2.vec[i], &chat);
        //poly_invntt_tomont(&cs2.vec[i]);
        index_based_sparse_binary_challenge_polymult_modQ(&cs2.vec[i], &s2.vec[i], &sp_c);
    }
