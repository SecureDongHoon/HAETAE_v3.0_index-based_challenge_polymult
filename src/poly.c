...
void extract_sparse_binpoly(sppoly *out, const poly *in) {
    size_t count = 0;
    for (size_t i = 0; i < N && count < TAU; i++) {
        if (in->coeffs[i] == 1) {
            out->coeffs[count++] = (uint8_t)i;
        }
    }
}

// [-Q/2, Q/2) 범위로 center 정규화된 값 생성
static inline int32_t centered_modQ_int64(int64_t x) {
    x %= Q;
    if (x > Q / 2) x -= Q;
    else if (x <= -Q / 2) x += Q;
    return (int32_t)x;
...
void index_based_sparse_binary_challenge_polymult_modQ(poly *r, const poly *a, const sppoly *sp_c) {
    int64_t acc[N] = {0};

    // sparse binary poly c의 각 1 항에 대해 dense poly a를 shift-accumulate
    for (size_t i = 0; i < TAU; i++) {
        uint8_t shift = sp_c->coeffs[i];
    ...
    // centered mod q 정규화 후 int32_t에 저장
    for (size_t i = 0; i < N; i++) {
        r->coeffs[i] = centered_modQ_int64(acc[i]);
    }
}
